...
// UK ONS time-series API (free JSON, no key) — used where FRED has no live
// feed, e.g. UK CPI after the OECD series were discontinued.
// Example: https://api.ons.gov.uk/timeseries/d7g7/dataset/mm23/data
async function fetchOnsSeries(cfg, timeoutMs = 30000) {
  const url = `https://api.ons.gov.uk/timeseries/${cfg.onsSeries}/dataset/${cfg.onsDataset}/data`;
  let lastErr;
  for (let attempt = 0; attempt < 3; attempt++) {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), timeoutMs);
