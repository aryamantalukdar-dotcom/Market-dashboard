...
// ECB Data Portal (free CSV, no key) — euro-area inflation-linked swap rates.
...
async function fetchEcbSeries(cfg, timeoutMs = 30000) {
  const url = `https://data-api.ecb.europa.eu/service/data/FM/${cfg.ecbKey}?format=csvdata&startPeriod=${startDate().slice(0, 7)}`;
  let lastErr;
  for (let attempt = 0; attempt < 3; attempt++) {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), timeoutMs);
...
async function fetchBoeSeries(cfg, timeoutMs = 30000) {
  ...
  const url = 'https://www.bankofengland.co.uk/boeapps/database/_iadb-fromshowcolumns.asp'
    ...
    + `&SeriesCodes=${cfg.boeCode}&CSVF=TN&UsingCodes=Y&VPD=Y&VFD=N`;
  let lastErr;
  for (let attempt = 0; attempt < 3; attempt++) {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), timeoutMs);
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const text = await res.text();
      if (/<html/i.test(text)) throw new Error('IADB returned an error page (bad series code?)');
      const rows = [];
      ...
      if (!rows.length) throw new Error('no observations in IADB response');
      return rows;
    } catch (err) {
      lastErr = err;
      await sleep(1000 * (attempt + 1));
...
export async function fetchMacroSeries(seriesConfigs) {
  ...
  await Promise.all(seriesConfigs.map(async (cfg) => {
    try {
      ...
      else if (cfg.source === 'boe') rows = await fetchBoeSeries(cfg);
