...
// Japan MoF JGB yield curve CSV (free, no key; western dates in the English
// files). Header: Date,1Y,2Y,...,40Y; rows like 2026/1/6,0.71,...
// The current-year file is tried first; the full-history file is the fallback.
const MOF_URLS = [
  'https://www.mof.go.jp/english/policy/jgbs/reference/interest_rate/jgbcme.csv',
  'https://www.mof.go.jp/english/policy/jgbs/reference/interest_rate/historical/jgbcme_all.csv'
];

async function fetchMofSeries(cfg, timeoutMs = 45000) {
  let lastErr;
  for (const url of MOF_URLS) {
    for (let attempt = 0; attempt < 2; attempt++) {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), timeoutMs);
      try {
        const res = await fetch(url, { headers: HEADERS, signal: ctrl.signal });
        if (!res.ok) {
          lastErr = new Error(`HTTP ${res.status} (${url.split('/').pop()})`);
          break; // 404 on this URL won't change — next candidate
        }
