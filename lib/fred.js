...
async function fetchCsv(id, timeoutMs = 15000) {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    const res = await fetch(BASE + encodeURIComponent(id), { headers: HEADERS, signal: ctrl.signal });
...
export async function fetchMacroSeries(seriesConfigs) {
  ...
  await Promise.all(seriesConfigs.map(async (cfg) => {
    try {
      const rows = parseCsv(await fetchCsv(cfg.id));
      let series = rows;
...
export function summarize(cfg, series) {
  const recent = series.slice(-400); // keep payload small
  const latest = recent[recent.length - 1] || null;
  const prior = recent[recent.length - 2] || null;
  // Trend: latest vs value ~3 observations back (3 months for monthly data)
  const back3 = recent[recent.length - 4] || null;
  const back6 = recent[recent.length - 7] || null;
  return {
    id: cfg.id,
    name: cfg.name,
    unit: cfg.unit,
    ...
    spark: recent.slice(-36).map((r) => r.value),
    updatedAt: Date.now()
  };
}
