...
// Escalating waits between chart rounds. Yahoo's rate limit is per-IP and
// time-based, not per-endpoint: once a burst trips it, /spark and /chart are
// both refused for a while. Waiting is therefore the only thing that recovers a
// blocked run, and a round returning zero symbols is the signal to wait longer
// rather than to give up.
const CHART_COOLDOWNS_MS = [30000, 60000, 120000, 180000];

// Fetch many symbols.
//
// Yahoo's /spark endpoint returns the whole universe in 7 requests, so it is
// tried first and abandoned quickly when refused. Everything else comes from
// per-symbol /chart, run in rounds separated by the cooldowns above.
//
...
export async function fetchAll(symbols, { range = '1y', concurrency = 4 } = {}) {
  ...
  // 2. Per-symbol /chart in rounds. If spark was refused outright the IP is
  //    already hot, so cool down before the first round too — going straight in
  //    just burns the round against a limit that is still in force.
  let hot = out.size === 0;
  let emptyRounds = 0;
  for (let round = 0; round < CHART_COOLDOWNS_MS.length; round++) {
    const missing = symbols.filter((s) => !out.has(s));
    if (!missing.length) break;
    if (hot) {
      const wait = CHART_COOLDOWNS_MS[round];
      console.warn(`[yahoo] ${missing.length} symbols missing — waiting ${wait / 1000}s for the rate limit to lift`);
      await sleep(wait);
    }
    ...
    // Partial progress means the limit is lifting; keep going without a wait.
    // No progress means it is not, and only time fixes that.
    hot = gained === 0;
    if (gained === 0) emptyRounds++; else emptyRounds = 0;
    // Observed on CI: when Yahoo blocks a runner IP it stays blocked for the
    // whole job, so further rounds only burn minutes. Two empty rounds is
    // enough to call it and let Stooq have the remaining time.
    if (emptyRounds >= 2) {
      console.warn('[yahoo] two empty rounds — Yahoo is blocked for this run, moving on');
      break;
    }
  }
  ...
  // Deliberately generous: the counter is shared by all workers, so a short
  // burst of failures at the start of the pass used to disable a source for
  // every remaining symbol. That alone is the difference between a run
  // fetching 41 instruments and one fetching 1.
