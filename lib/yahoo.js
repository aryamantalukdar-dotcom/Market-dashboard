...
async function fetchChartViaProxies(symbol, range, interval) {
  ...
  for (const wrap of PROXIES) {
    try {
      const json = await fetchJson(wrap(chartUrl(HOSTS[0], symbol, range, interval)), 15000);
      return parseChartResult(symbol, json?.chart?.result?.[0]);
    } catch (err) {
      lastErr = err;
    }
...
async function sparkPass(symbols, range, out, wrap) {
  for (let i = 0; i < symbols.length; i += 20) {
    ...
    } catch { /* whole batch failed; later passes retry it */ }
  }
}
...
export async function fetchAll(symbols, { range = '1y', concurrency = 4 } = {}) {
  ...
  for (const wrap of PROXIES) {
    ...
    await sparkPass(missing, range, out, wrap);
  }
