...
// from their own egress IPs. Only price data flows through them.
const PROXIES = [
...
function parseChart(symbol, json) {
...
export async function fetchChart(symbol, range = '1y', interval = '1d', attempts = 2) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    try {
      return parseChart(symbol, await fetchJson(url));
    } catch (err) {
      lastErr = err;
...
async function fetchChartViaProxies(symbol, range, interval) {
  ...
  for (const wrap of PROXIES) {
    try {
      return parseChart(symbol, await fetchJson(wrap(chartUrl(HOSTS[0], symbol, range, interval)), 15000));
    } catch (err) {
      lastErr = err;
    }
  }
...
// Fallback chain per symbol: Yahoo direct -> Stooq -> Yahoo via relay proxy.
// Circuit breakers skip sources that fail repeatedly (e.g. on CI runner IPs)
// so a fully-blocked source doesn't burn the whole time budget.
