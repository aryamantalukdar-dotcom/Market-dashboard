...
async function sparkPass(symbols, range, out, relay) {
  ...
  // Yahoo rate-limits bursts, so unspaced back-to-back batches reliably trip a
  // 429 that has nothing to do with the daily quota. Space them and retry with
  // backoff — the limiter window is short.
  const attempts = relay ? 2 : 3;
  const gapMs = relay ? 1200 : 800;
  let batchFails = 0;
  const sizeAtStart = out.size;
  for (let i = 0; i < symbols.length; i += 10) {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      try {
        ...
        if (attempt + 1 < attempts) await sleep(1500 * 2 ** attempt + Math.random() * 500);
      }
    }
    // A relay that fails its first two batches outright is down, not throttled;
    // plowing through the rest costs minutes and yields nothing.
    if (!ok && relay && ++batchFails >= 2 && out.size === sizeAtStart) {
      console.warn(`[yahoo] relay ${via} appears down — skipping remaining batches`);
      return;
    }
    await sleep(gapMs);
  }
...
export async function fetchAll(symbols, { range = '1y', concurrency = 4 } = {}) {
  ...
  // Direct Yahoo is the only source that reliably works from CI (Stooq and the
  // public relays both block/throttle datacenter IPs), so exhaust it first: a
  // 429 here is a short burst limit, and waiting it out beats any fallback.
  for (let pass = 0; pass < 3; pass++) {
    const missing = symbols.filter((s) => !out.has(s));
    if (!missing.length) break;
    if (pass > 0) {
      console.warn(`[yahoo] ${missing.length} symbols still missing — cooling down before direct retry ${pass}`);
      await sleep(45000);
    }
    await sparkPass(missing, range, out, null);
  }
