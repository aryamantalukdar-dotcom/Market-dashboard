...
export function classifyRegime(indicators, macro) {
...
function macroAdjustments(regime) {
  const adj = {};
  const why = {};
...
export function buildRecommendations({ indicators, macro, newsAgg, history, now = Date.now() }) {
  const regime = classifyRegime(indicators, macro);
  const bench = get(indicators, 'ACWI');
  const macroAdj = macroAdjustments(regime);
