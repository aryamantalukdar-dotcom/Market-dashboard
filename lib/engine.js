...
// A policy-rate series that stopped updating must not drive tilts.
function freshLevel(series, maxAgeDays = 200) {
  if (!series || series.latest == null || !series.latestDate) return null;
  if (Date.now() - Date.parse(series.latestDate) > maxAgeDays * 86400 * 1000) return null;
  return series;
}

function macroAdjustments(regime, policy = null, macro = new Map()) {
  const adj = {};
  const why = {};
  ...
  // Foreign central-bank stance (delivered moves over the last 6 months)
  const ecb = freshLevel(macro.get('ECBDFR'));
  if (ecb?.change6m != null) {
    if (ecb.change6m <= -0.5) add('europe', 0.08, 'ECB easing cycle is a tailwind for European assets');
    else if (ecb.change6m >= 0.5) add('europe', -0.06, 'ECB tightening is a headwind for European assets');
  }
  const boe = freshLevel(macro.get('IUDSOIA'));
  if (boe?.change6m != null) {
    if (boe.change6m <= -0.5) add('uk', 0.08, 'BoE easing cycle is a tailwind for UK assets');
    else if (boe.change6m >= 0.5) add('uk', -0.06, 'BoE tightening is a headwind for UK assets');
  }
  const boj = freshLevel(macro.get('IRSTCI01JPM156N'));
  if (boj?.change6m != null && boj.change6m >= 0.25) {
    add('japan', -0.08, 'BoJ tightening / stronger-yen headwind for Japanese equities');
  }
...
export function buildRecommendations({ indicators, macro, newsAgg, history, policy = null, now = Date.now() }) {
  ...
  const macroAdj = macroAdjustments(regime, policy, macro);
