...
const PROFILES = [
  ...
  // Fed funds futures: price ~ 100 - implied rate, so keep it tightly anchored
  [/^ZQ.*\.CBT$/, { start: 96.3, drift: 0.00004, vol: 0.0008, floor: 90, exact: true }],
...
export function mockIndicators() {
  ...
  for (const inst of allInstruments()) {
    ...
    const closes = [];
    const times = [];
    let price = p.start * (p.exact ? 1 + (rand() - 0.5) * 0.01 : 0.5 + rand());
    for (let i = 252; i > 0; i--) {
...
const MOCK_MACRO_BASE = {
  ...
  INDPRO: { start: 1.2, step: 0.01 },
  ICSA: { start: 225, step: 0.4 },
  ECBDFR: { start: 2.0, step: 0 },
  IUDSOIA: { start: 4.0, step: -0.03 },
  IRSTCI01JPM156N: { start: 0.4, step: 0.012 },
  CP0000EZ19M086NEST: { start: 2.2, step: -0.02 },
  UKCPI_D7G7: { start: 2.8, step: -0.025 },
  T5YIE: { start: 2.35, step: -0.005 },
  T10YIE: { start: 2.3, step: -0.004 },
  T5YIFR: { start: 2.25, step: -0.002 },
  EZBE5: { start: 2.05, step: -0.004 },
  EZBE10: { start: 2.15, step: -0.003 },
  UKBE5: { start: 3.3, step: -0.005 },
  UKBE10: { start: 3.5, step: -0.004 },
  EZIF1Y: { start: 2.3, step: -0.003 },
  JGB1Y: { start: 0.85, step: 0.004 }
};

// Series length and date spacing per frequency; per-step drift is scaled so
// the total drift over the window roughly matches the monthly intent.
const MOCK_FREQ = {
  m: { n: 37, stepDays: 0, stepScale: 1 },
  w: { n: 140, stepDays: 7, stepScale: 0.25 },
  d: { n: 320, stepDays: 1, stepScale: 0.05 },
  q: { n: 16, stepDays: 91, stepScale: 3 }
};
...
export function mockMacro() {
  ...
  for (const cfg of FRED_SERIES) {
    ...
    const f = MOCK_FREQ[cfg.freq] || MOCK_FREQ.m;
    const rand = rng(hashCode(cfg.id));
    const series = [];
    const today = new Date();
    for (let i = f.n - 1; i >= 0; i--) {
      const d = f.stepDays
        ? new Date(today.getTime() - i * f.stepDays * 86400 * 1000)
        : new Date(today.getFullYear(), today.getMonth() - i, 1);
      series.push({
        date: d.toISOString().slice(0, 10),
        value: +(base.start + base.step * f.stepScale * (f.n - 1 - i) + (rand() - 0.5) * Math.abs(base.start) * 0.05).toFixed(2)
      });
    }
